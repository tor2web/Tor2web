...
"""Blocking and non-blocking HTTP client implementations using pycurl, with
support for SOCKS"""
...
class HTTPLib2AsyncHTTPClient(AsyncHTTPClient):
    ...
    def close(self):
        ...
        self._multi.close()
        self._closed = True
        super(HTTPLib2AsyncHTTPClient, self).close()
...
def _curl_setup_request(curl, request, buffer, headers):
    ...
    if request.proxy_host.startswith("socks"):
        curl.setopt(pycurl.PROXY, request.proxy_host.split("/")[-1])
        ...
        try:
            ...
            pass
    elif request.proxy_host and request.proxy_port:
...
if __name__ == "__main__":
    AsyncHTTPClient.configure(HTTPLib2AsyncHTTPClient)
    main()
